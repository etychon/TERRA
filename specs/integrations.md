...
- **Syslog / alarms / audit streams:** Manager **alarms, events, and audit** are ingested via **`collector`** into Postgres (`sdwan_governance_events`) — see [`specs/governance-events.md`](governance-events.md). **Syslog push** to SIEM remains out of scope for REST ingest. Distinguish **network events** from **dashboard RBAC audit** (Postgres-backed durable trail — not the admin ring buffer).
