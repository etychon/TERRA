...
- **Per-tenant inventory:** For each tenant with a resolvable switch id, TERRA calls **`POST /dataservice/tenant/{tenantId}/switch`** then **`GET /dataservice/device`** (same fallbacks as single-tenant). Session cookies and `X-XSRF-TOKEN` from `open_manager_http_client` apply to the switch POST.
- **JWT + multitenant:** Bearer tokens still need **`X-XSRF-TOKEN`** aligned with the Manager session. TERRA refreshes it from **`GET /dataservice/client/server`** (`data.CSRFToken` or `data[0].CSRFToken`) before tenant switches. Do **not** send `VSessionId` on `/dataservice/device` after switch (that path can return an empty list on some builds); rely on switch + cookies for inventory GETs.
...
The primary table TERRA uses today is **`GET /dataservice/device/interface`** (and **`.../interface/synced`**), merged into inventory as `deviceInterface` (`src/terra/sdwan_device_live.py`).
